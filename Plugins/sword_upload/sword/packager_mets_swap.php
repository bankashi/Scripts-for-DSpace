...
class PackagerMetsSwap {
    ...
    // Identifier
    public $sac_identifiers;

    //IdenfierUri
    public $sac_identifierUri;
    ...
    function __construct($sac_rootin, $sac_dirin, $sac_rootout, $sac_fileout) {
        ...
        $this->sac_types = array();
        $this->sac_files = array();
        $this->sac_mimetypes = array();
        $this->sac_provenances = array();
        $this->sac_identifiers = array();
        $this->sac_rights = array();
        $this->sac_filecount = 0;
    }

    function setIdentifierUri($sac_theidentifieruri) {
        $this->sac_identifierUri = $sac_theidentifieruri;
    }

    function setRightsUri($sac_rightsuri) {
        $this->sac_rightsUri = $sac_rightsuri;
    }
    ...
    function setAbstract($sac_theabstract) {
        $this->sac_abstract = $this->clean($sac_theabstract);
    }

    function setDescription($sac_thedescription) {
        $this->sac_description = $this->clean($sac_thedescription);
    }
    ...
    function addTypes($sac_type) {
        array_push($this->sac_types, $this->clean($sac_type));
    }
    ...
    function addIdentifier($sac_identifier) {
        array_push($this->sac_identifiers, $this->clean($sac_identifier));
    }

    function setRelation($sac_therelation) {
        $this->sac_relation = $sac_therelation;
    }
    ...
    function setHasPart($sac_thehaspart) {
        $this->sac_haspart = $this->clean($sac_thehaspart);
    }
    ...
    function writeDmdSec($fh) {
        ...
        if (isset($this->sac_identifierUri)) {
            $this->statement($fh,
                "http://purl.org/dc/terms/identifierUri",
                $this->valueString($this->sac_identifierUri));
        }
        ...
        if (isset($this->sac_description)) {
            $this->statement($fh,
                "http://purl.org/dc/elements/1.1/description",
                $this->valueString($this->sac_description));
        }
        ...
        foreach ($this->sac_types as $sac_type) {
            $this->statement($fh,
                "http://purl.org/dc/elements/1.1/type",
                $this->valueString($sac_type));
        }
        ...
        if (isset($this->sac_rightsUri)) {
            $this->statement($fh,
                "http://purl.org/dc/terms/rightsUri",
                $this->valueString($this->sac_rightsUri));
        }

        foreach ($this->sac_identifiers as $sac_identifier) {
            $this->statement($fh,
                "http://purl.org/dc/elements/1.1/identifier",
                $this->valueString($sac_identifier));
        }

        if (isset($this->sac_relation)) {
            $this->statement($fh,
                "http://purl.org/dc/elements/1.1/relation",
                $this->valueString($this->sac_relation));
        }

        if (isset($this->sac_haspart)) {
            $this->statement($fh,
                "http://purl.org/dc/terms/hasPart",
                $this->valueString($this->sac_haspart));
        }
